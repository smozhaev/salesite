...
class DiscountSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        if data["sale_date_start"] > data["sale_date_end"]:
            raise serializers.ValidationError(
                "Дата начала скидки не может быть позднее даты окончания."
            )
        return data
    ...
    def validate_description(self, value):
        if len(value) > 300:
            raise serializers.ValidationError(
                "Описание слишком длинное. Максимальная длина - 300 символов."
            )
        return value
    ...
    def validate_title(self, value):
        ...
        if len(value) > 100:
            raise serializers.ValidationError(
                "Название слишком длинное. Максимальная длина - 100 символов."
            )
        return value
...
class CompanySerializer(serializers.ModelSerializer):
    ...
    def validate_description(self, value):
        if len(value) > 500:
            raise serializers.ValidationError(
                "Описание слишком длинное. Максимальная длина - 500 символов."
            )
        return value
