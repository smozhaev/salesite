...
class DiscountApiView(generics.ListAPIView):
    ...
    filterset_fields = ['categories', 'company', 'tags']  # Предполагается, что у модели Discount есть поле categories
    search_fields = ['title', 'description', 'company__name']  # Добавьте сюда поля, по которым вы хотите осуществлять поиск
    ...
    def get_queryset(self):
        ...
        queryset = queryset.filter(
            (Q(sale_date_start__range=[datetime.date(2023, 1, 1), datetime.date(2023, 12, 31)]) |
             Q(sale_date_end__range=[datetime.date(2024, 1, 1), datetime.date(2024, 1, 31)])) &
...
class DiscountViewSet(viewsets.ModelViewSet):
    ...
    def statistics(self, request):
        ...
        category_stats = self.queryset.values('categories__name').annotate(count=Count('categories')).order_by('-count')
    ...
    def update_description(self, request, pk=None):
        ...
        return Response({'error': 'Новое описание не предоставлено'}, status=status.HTTP_400_BAD_REQUEST)
...
class CompanyApiView(generics.ListAPIView):
    ...
    filter_backends = [DjangoFilterBackend, SearchFilter, OrderingFilter]
    search_fields = ['name']  # Добавьте сюда поля, по которым вы хотите осуществлять поиск
